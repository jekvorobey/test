...
export function addToMap(markers, { options, callbacks, map, useObjectManager, objectManagerClusterize }) {
    const defaultLayout = `
    ...
    Object.keys(clusters).forEach(clusterName => {
        ...
        clusterOptions.clusterIconContentLayout =
            clusterIconContentLayout && ymaps.templateLayoutFactory.createClass(clusterIconContentLayout);
...
export function objectComparison(first, second) {
    ...
    function inCache(f, s) {
        let i = cache.length;
        while (i--) {
            if ((cache[i][0] === f || cache[i][0] === s) && (cache[i][1] === s || cache[i][1] === f)) return true;
        }
        return false;
    }
...
export function ymapLoader(settings = {}) {
    return new Promise((res, rej) => {
        ...
        const { apiKey = '', lang = 'ru_RU', version = '2.1', coordorder = 'latlong', debug = false } = settings;
